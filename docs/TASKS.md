...
- **Firestore セキュリティルール設定**（`firestore.rules`、`firebase.json`、`firestore.indexes.json`）
- **IndexedDB キャッシュ戦略実装**（24時間キャッシュ、フォールバック機能）
- **宣言的ルールエンジン実装**（`core_rules.js`、`rule_test.html`）
- **データ移行準備完了**（管理者画面から手動登録可能）
- **VBA連携機能実装完了**（Excelボタンホバー修正、クリップボード形式シンプル化）
- **計算ロジック修正完了**（カビ、MJ60新などareaThresholdなし商品の計算修正）

## 次回の優先タスク（提案）
1) **Firebase ConsoleでFirestoreルールを手動設定**（管理者メールの登録）
2) **管理者画面からdata.js商品を順次登録**（スキーマ変換済み）
3) **Feature FlagでFirestore切替テスト**（`FEATURE_USE_FIRESTORE=true`）
4) **宣言的ルールの統合テスト**（計算コアとの連携）
5) **本番環境での動作確認**（キャッシュ・フォールバック・ルール適用）

---

## 成果物チェックリスト
- [x] **Firestore セキュリティルール設定完了**（`firestore.rules`）
- [x] **IndexedDB キャッシュ戦略実装完了**（24時間キャッシュ）
- [x] **宣言的ルールエンジン実装完了**（`core_rules.js`）
- [x] **管理者画面CRUD機能完了**（登録/編集/削除/公開切替）
- [x] **80件の商品データ移行完了**（通常商品 + 基礎商品）
- [x] **メインアプリケーション連携完了**（Calculation.html）
- [x] **基礎商品計算機能完了**（高さ・長さによる動的計算）
- [x] **キャッシュ問題解決完了**（IndexedDBキャッシュクリア）
- [x] **旧/新の計算結果が主要ケースで一致**
- [x] **価格データ変更が管理画面から反映（Draft→Publish）**
- [x] **セキュリティルールで一般ユーザは published のみ参照**
- [x] **VBA連携機能完了**（Excelボタンホバー修正、クリップボード形式シンプル化）
- [x] **計算ロジック修正完了**（カビ、MJ60新などareaThresholdなし商品の計算修正）
- [ ] 逆チェックは新コアを使用し計算一致
- [ ] 回帰テストがCIで自動実行
