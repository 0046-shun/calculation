...
- **サマリーとセット割** `updateSummary()`
  ...
  - 固定額オプション（`fixed_fees.js` の `FixedFees.options` で一元管理。チェックオンで税抜合計へ固定額を加算）:
    - 一般管理費（管有）: デフォルトON。商品名末尾に「・管有」、税抜+20,000、税込+22,000 を加算。
    - 離島手数料: デフォルトOFF。チェックオンで商品名リストにトークンを追加し、以下の金額を加算。
      - 島特1: +30,000
      - 離島1: +20,000
      - 離島2: +20,000
      - 離島3: +5,000
    - 追加・変更は `fixed_fees.js` の定義配列を編集するだけでよい（UI生成・合計計算・Excel連携・逆計算チェックが自動で追従）。
